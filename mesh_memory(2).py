...
def draw_heat_map(heat_map, name):
    
    global fig_count
    heat_map = heat_map[0][0].detach().cpu().numpy()
    plt.imshow(heat_map, cmap='hot', interpolation='nearest')
    plt.colorbar()
    plt.savefig(f'{name}_score_matrix{fig_count}.png')
    plt.show()
...
def attention(query, key, value, mask=None, dropout=None, name = ''): # [batch_size, head, 98, dim_perhead(64)]
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(mask==0, float('-inf'))
    # draw_heat_map(scores, name)
...
class ImageMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, Mk, Mv, mask=None):
        ...
        x, self.attn = attention(query, key, value, mask=mask, dropout=self.dropout,name = __name__)
...
class SelfMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        # exit()
        if layer_past is not None:
            return self.linears[-1](self.dropout(F.relu(self.linears[-2](x)))), present
        else:
            return self.linears[-1](self.dropout(F.relu(self.linears[-2](x))))
...
class Encoder(nn.Module):
    def __init__(self, layer, args):
        ...
        self.mm_mtr = {}
        # self.mm_mtr['key'] = nn.Parameter(torch.FloatTensor(1, 20, self.dim)).cuda()
        # self.mm_mtr['value'] = nn.Parameter(torch.FloatTensor(1, 20, self.dim)).cuda()
        ...
        nn.init.normal_(self.mm_mtr['key'], 0, 1 / self.dim)
...
class BaseCMN(AttModel):
    def make_model(self, tgt_vocab, cmn):
        ...
        image_attn = ImageMultiHeadedAttention(self.num_heads, self.d_model, self.args.dropout)
...
class SrcMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        # exit()
        x = (torch.sum(sigma * x, dim = 1) + query[:, 0]) / np.sqrt(3) #### 1111!!!
