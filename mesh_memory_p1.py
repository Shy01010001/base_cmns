...
class Encoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.mm_mtr['key'] = torch.stack([nn.Parameter(torch.FloatTensor(20, 512)) for _ in range(self.N)]).cuda()
        self.mm_mtr['value'] = torch.stack([nn.Parameter(torch.FloatTensor(20, 512)) for _ in range(self.N)]).cuda()
        nn.init.normal_(self.mm_mtr['key'], 0, 1 / 512)
        nn.init.normal_(self.mm_mtr['value'], 0, 1 / 20)
    def forward(self, x, mask):
        record = torch.zeros_like(x)
        for i, layer in enumerate(self.layers):
            x = layer(x, mask, self.mm_mtr['key'][i, :, :], self.mm_mtr['value'][i, :, :])
            if i == 0:
                record = x
            else:  
                record = torch.cat((record, x), dim = 0)        
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        if type(x) is tuple:
            return self.norm(x + self.dropout(x[0])), x[1]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, Mk, Mv):
        x = self.self_attn(x, Mk, Mv, mask)
    ...
    def forward(self, x, memory, src_mask, tgt_mask, past=None):
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past)):
            ...
            if layer_past is not None:
                ...
                x = self.sublayer(x[0], self.fw)
            else:
                x = self.sublayer(x, self.fw)
        if past[0] is None:
            return x
        else:
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, layer_past=None):
        ...
        if layer_past is None:
            ...
            present = [None, None]
            x, present[0] = self.self_attn(x, x, x, tgt_mask, layer_past[0])
            x, present[1] = self.src_attn(x, m, m, src_mask, layer_past[1])
            return self.sublayer(x, self.feed_forward), present
...
class ImageMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, Mk, Mv, mask=None):
        if mask is not None:
            mask = mask.unsqueeze(1)
        nbatches = query.size(0)
        query, key, value = \
            [l(x) for l, x in zip(self.linears, (query, query, query))]
        Mk = np.sqrt(self.d_model) * Mk.unsqueeze(0).repeat(key.size(0), 1, 1)
...
class SrcMultiHeadedAttention(nn.Module):
    def __init__(self, head, d_model, N, dropout=0.1):
        ...
        self.key_map = clones(nn.Linear(d_model, d_model), self.layer_num)
        self.value_map = clones(nn.Linear(d_model, d_model), self.layer_num)
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        x = x.transpose(1, 2).contiguous().view(nbatches, self.layer_num, -1, self.h * self.d_k)
...
class BaseCMN(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn):
        ...
        aug_attn = ImageMultiHeadedAttention(self.num_heads, self.d_model)
        src_attn = SrcMultiHeadedAttention(self.num_heads, self.d_model, self.num_layers)
...
class ImageMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, Mk, Mv, mask=None):
        ...
        query, key, value = [l(x) for l, x in zip(self.linears, (query, query, query))]
        Mk = Mk.unsqueeze(0).repeat(key.size(0), 1, 1)
        Mv = Mv.unsqueeze(0).repeat(key.size(0), 1, 1)
        key = torch.cat((key, Mk), dim = 1)
        value = torch.cat((value, Mv), dim = 1)
...
class TokenFussion(nn.Module):
    def __init__(self, d_model, num_classes, dropout = 0):
        ...
        self.attn1 = nn.Sequential(nn.Linear(self.input_dimension, 4096), nn.ReLU(), nn.Linear(4096, num_classes), nn.ReLU())
        self.norm = LayerNorm(512)
    def forward(self, x):
        x = self.norm(x)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, Mk, Mv):
        x = self.sublayer[0](x, lambda x: self.self_attn(x, Mk, Mv, mask)) # attn: [batch_size, head, 98, 98]
...
class Encoder(nn.Module):
    def __init__(self, layer, args):
        ...
        self.layers = clones(layer, self.N) # N个encoder layer
        self.norm = LayerNorm(layer.size)
        self.mm_mtr = {}
        self.mm_mtr['key'] = torch.stack([nn.Parameter(torch.FloatTensor(20, self.dim)) for _ in range(self.N)]).cuda()
        self.mm_mtr['value'] = torch.stack([nn.Parameter(torch.FloatTensor(20, self.dim)) for _ in range(self.N)]).cuda()
        # self.mm_mtr['key'] = torch.stack([nn.Parameter(torch.FloatTensor(20, self.dim)) for _ in range(self.N)]).detach()
        # self.mm_mtr['value'] = torch.stack([nn.Parameter(torch.FloatTensor(20, self.dim)) for _ in range(self.N)]).detach()
        for key in self.mm_mtr:
            for mm_dim in range(self.N):
                nn.init.normal_(self.mm_mtr[key][mm_dim], 0, 1 / self.dim)  
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, past=None): # report词向量[batch_size, seq_len, d_model], encoder输出[batch_size, 98, d_model]
        ...
        # memory size: b x layer_num x 98 x 512
        if past is not None:
            present = [[], []]
            x = x[:, -1:]
            tgt_mask = tgt_mask[:, -1:] if tgt_mask is not None else None
...
class BaseCMN(AttModel):
    def make_model(self, tgt_vocab, cmn):
        c = copy.deepcopy
        self_attn = SelfMultiHeadedAttention(self.num_heads, self.d_model)
        src_attn = SrcMultiHeadedAttention(self.num_heads, self.d_model, self.num_layers)
        
        image_attn = ImageMultiHeadedAttention(self.num_heads, self.d_model)
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        position = PositionalEncoding(self.d_model, self.dropout)
        model = Transformer(
            Encoder(EncoderLayer(self.d_model, c(image_attn), c(ff), self.dropout), self.args), # only 1 layer for image feature extraction
            Decoder(DecoderLayer(self.d_model, c(self_attn), c(src_attn), c(ff), self.dropout), self.num_layers),
    ...
    def __init__(self, args, tokenizer):
        ...
        self.token_fussion = TokenFussion(512, 50)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks)
        att_masks = None
        img_ftr = self.model.encode(att_feats, att_masks)

        return fc_feats[..., :1], att_feats[..., :1], img_ftr, att_masks
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None): # seq: (batch_size, seq_len)
        ...
        att_masks = None
        out = self.model(att_feats, seq, att_masks, seq_mask) # [batch_size, seq_len, d_model]
        outputs = F.log_softmax(self.logit(out), dim=-1) # [batch_size, max_seq_len-1, vocab_size+1]

        return outputs

    def core(self, it, fc_feats_ph, att_feats_ph, img_ftr, state, mask):
        memory = img_ftr
        # print(fc_feats_ph.size())
        # exit()
        if len(state) == 0:
...
class SrcMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, layer_past=None):
        ...
        sigma = torch.zeros_like(query)
        for lyr in range(self.layer_num):
            sigma[:, lyr] = self.sigma_map[lyr](torch.cat((query[:, lyr], x[:, lyr]), dim = -1))
        # print('qual?',torch.equal(query[:, 0], query[:, 1]))
        # exit()
        x = self.norm(torch.sum(sigma * x, dim = 1) + query[:, 0]) #### 1111!!!
        # print('prensentL:',present.size())
        if layer_past is not None:
            return self.fw_map[-1](x), present
        else:
            return self.fw_map[-1](x)
        
# if __name__ == '__main__':
    
#     query = torch.rand(2, 39, 512)
#     key = torch.rand(2,3,98,512)
#     test_module = SrcMultiHeadedAttention(8, 512, 3)
#     test_module(query, key, key)
    
>>>>>>> aa969439b333a738a1c74ae78fc0d52e2c3910f9
