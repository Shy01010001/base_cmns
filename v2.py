...
class Encoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.mm_mtr['key'] = torch.stack([nn.Parameter(torch.FloatTensor(40, 512)) for _ in range(self.N)]).cuda()
        self.mm_mtr['value'] = torch.stack([nn.Parameter(torch.FloatTensor(40, 512)) for _ in range(self.N)]).cuda()
...
class ImageMultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, Mk, Mv, mask=None):
        ...
        Mv = np.sqrt(20) * Mv.unsqueeze(0).repeat(key.size(0), 1, 1)
